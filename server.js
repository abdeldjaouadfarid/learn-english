...
if (!process.env.SMTP_USER) {
  console.log(
    "[info] SMTP not configured — password-reset links will print to console.",
  );
}
...
app.post("/api/test/submit", requireAuth, async (req, res) => {
  try {
    ...
    await query("UPDATE sessions SET status = $1 WHERE id = $2", [
      "done",
      session_id,
    ]);
...
app.get("/api/session/:id", requireAuth, async (req, res) => {
  ...
  const resultRes = await query("SELECT * FROM results WHERE session_id = $1", [
    req.params.id,
  ]);
  ...
  res.json({
    ...
    result: resultRes.rows[0]
      ? {
          ...resultRes.rows[0],
          skills: JSON.parse(resultRes.rows[0].skills_json),
        }
      : null,
    roadmap: roadmapRes.rows[0]
      ? JSON.parse(roadmapRes.rows[0].content_json)
      : null,
  });
});
...
app.post("/api/vocab/generate", requireAuth, async (req, res) => {
  try {
    ...
    const existingRes = await query(
      "SELECT word FROM vocab_words WHERE user_id = $1",
      [req.user.id],
    );
    const existing = existingRes.rows.map((r) => r.word.toLowerCase());
    const existingSet = new Set(existing);

    const candidates = await generateVocabBatch({
...
app.post("/api/vocab/submit", requireAuth, async (req, res) => {
  try {
    ...
    if (!Array.isArray(known) || !Array.isArray(unknown)) {
      return res
        .status(400)
        .json({ error: "known and unknown must be arrays" });
    }
    ...
    res.json({
      evaluation,
      saved: { known: known.length, unknown: unknown.length },
    });
  } catch (err) {
...
app.get("/api/vocab/unknown", requireAuth, async (req, res) => {
  ...
  const missing = rows.filter(
    (r) => r.arabic == null || r.example_sentence == null,
  ).length;
